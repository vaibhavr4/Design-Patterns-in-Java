...
public class BitVectorTests {
	...
	/**
	 * method to test Iterator of BitVector
	 */
	@Test
	public void testiterator()
	{
		BitVector bi1 = new BitVector();
    	int arr[] = {1,4};
    	for(int i=0;i<2;i++)
			bi1.set(arr[i]);
    	Iterator<Integer> itr = bi1.iterator();   		
		assertTrue(itr.hasAnotherElement());
		BitVector bi2 = new BitVector();
		bi2.copy(bi1);
		assertEquals(itr.nextElement(),bi2.iterator().nextElement());
		BitVector bi3 = new BitVector();
		int ar[] = {1,2,3};
    	for(int i=0;i<3;i++)
			bi3.set(ar[i]);
		Iterator<Integer> it = bi3.iterator();
		while (it.hasAnotherElement()) {
			it.nextElement();
		}
		assertFalse(it.hasAnotherElement());
		assertEquals(null,it.nextElement());
